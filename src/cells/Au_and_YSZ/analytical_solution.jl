...
function get_AYA_Sl_analytical_poisson_eq(data, V_tot, V_ISR; verbose=true, return_nFs=false)
...
function get_AYA_Sl_semianalytical_V_ISR(data, V_tot)
    g = get_AYA_Sl_analytical_poisson_eq(data, V_tot, 666, verbose=false)
...
function get_V_ISR_error(STcell::AYA_Sl)
    ...
    phi_L = STcell.U[1, 1]
    phi_R = STcell.U[1, Int(round(end/2))]
    V_tot = phi_L - phi_R
    V_ISR = phi_ISR - phi_R
    #@show phi_R
    rel_phi_err = (get_AYA_Sl_semianalytical_V_ISR(data, V_tot) - V_ISR)/V_ISR
    return rel_phi_err
end

function biaswise_V_ISR_errors(STcell::AYA_Sl)
    function perform_it(bias, STcell)
        if !test_partial_charges(STcell)
            throw(TestError("TestError: Partial charges are not allright"))
        end
        rel_phi_err = get_V_ISR_error(STcell)
        #@show bias, rel_phi_err
        push!(U_storage, deepcopy(STcell.U))
        push!(rel_err_storage, rel_phi_err)
    end

    rel_err_storage = []
    U_storage = []
    biasshow!(STcell, bend=0.3, bstep=0.01, tend=1e-0, callback=perform_it)
